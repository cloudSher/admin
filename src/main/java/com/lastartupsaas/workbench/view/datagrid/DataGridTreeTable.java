 ...
 * @date 2017-01-03
 */
public class DataGridTreeTable extends VerticalLayout implements PagingNavigator.PageItemClickListener, Button.ClickListener {
	...
	public DataGridTreeTable(DataGridModel gridModel) {
		super();
	...
	private void createTable() {
		...
		for (DataGridColumn column : this.model.getGridColumns()) {
			...
			treeTable.addContainerProperty(column.getId(), column.getType(), column.getDefaultValue(), column.getCaption(), null, Table.Align.LEFT);
		}
		if (this.model.getItemActionCommands().size() > 0) {
			treeTable.addContainerProperty("__ACTION__", HorizontalLayout.class, false, "操作", null, Table.Align.CENTER);
		}
	...
	protected void buildTreeTable(Object parentItemId, List<?> itemList) {
		for (Object itemObj : itemList) {
			final DataGridRow row = this.model.getGridRowConverter().convertRowData(itemObj);
			if (row != null) {
				List<Object> ls = new ArrayList<Object>();
				if (this.model.getListActionCommands().size() > 0) {
					ls.add(new CheckBox());
				...
				treeTable.addItem(ls.toArray(), row.getRowId());
				treeTable.setParent(row.getRowId(), parentItemId);
				if (row.getChildList() != null && row.getChildList().size() > 0) {
					buildTreeTable(row.getRowId(), row.getChildList());
				} else {
					treeTable.setChildrenAllowed(row.getRowId(), false);
				}
			}
		}
	}
	...
	public void showAllRows(boolean showAll) {
		if (showAll) {
			...
			this.treeTable.setPageLength(15);
		}
	}
